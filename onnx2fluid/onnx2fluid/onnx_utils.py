...
def node_topo(nodes,
    ...
    node_topo = []
    node_in_degrees = [len(set(node.input)) for node in nodes] # merge multiple references
    node_out_degrees = [len(set(node.output)) for node in nodes] # merge multiple references
    ...
    for initializer in model.graph.initializer:
        ...
        if name in input_refs:
            ...
            logger.debug('initializer %s(%s[%d]) stripped',
                         name, dtype, len(initializer.raw_data) // dtype.itemsize)
    ...
    for item in model.graph.input:
        ...
        if name in input_refs or name in out_names:
            ...
            logger.debug('input %s(%s%s) stripped',
                         name, tensor_dtype(item), tuple(tensor_shape(item)))
    return ret

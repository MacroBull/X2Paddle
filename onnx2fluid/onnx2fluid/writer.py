...
#                    raise ValueError('unsupported attribute {} = {}'.format(key, value))
    ...
    def Op(self, domain, op_type, inputs, outputs, attrs,
           *args, **kwargs):
        """
        ...
        if op_type in symbolic.DEFAULT_OP_MAPPING:
            symbolic._default(self, op_type, inputs, outputs, attrs, *args, **kwargs)
    ...
    def IntermediateOp(self, domain, op_type, inputs, outputs, attrs,
                       *args, **kwargs):
        """
    ...
    def VarTypeShapeInfo(self, name, value_info,
                         remove_batch=None):
        """
...
class Writer(object):
    ...
    def emit_param(prog, name, value_info):
        ...
        if embedded_names:
            prog.Code('# parameter {} embedded as {}'.format(name, embedded_names))
            for embedded_name in embedded_names:
                prog.VarDesc(embedded_name, persistable=True, value_info=value_info)
        else:
    ...
    def write_weight(
        ...
        assert lod is None or isinstance(lod, list), 'lod should be None or list'
